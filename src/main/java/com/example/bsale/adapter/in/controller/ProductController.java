...
public class ProductController {
    ...
    public Mono<Map<String, Object>> multiPart(@RequestPart("valHash") String valHash, @RequestPart("archivo") FilePart file) throws IOException {
        File document = File.createTempFile("trama", ".zip");
        StringJoiner binaryData = new StringJoiner(" ");

        return file.transferTo(document)
                .then(Mono.just(document)
                        .map(doc -> {
                            try (InputStream inputData = new FileInputStream(doc);
                            ...
                            data.put("archivo-contest", doc.length());
